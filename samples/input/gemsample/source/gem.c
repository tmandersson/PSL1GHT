...
static inline void initAttributeGem(gemAttribute* attribute, u32 max_connect,void * memory_ptr,	Spurs *spurs, const u8 spu_priorities[8])
{
	...
	for(i=0; i<8; ++i) attribute->spu_priorities[i] = spu_priorities[i];
}
int initGem()
{
	int ret;
	...
	u8 gem_spu_priorities[8] = {1,0,0,0,0,0,0,0}; // execute libgem jobs on 1 spu
	gemAttribute gem_attr;
...
{	
	...
	printf(" accelerometer device coordinates [%f,%f,%f,%f]\n",vec_array(position,0),vec_array(position,1),vec_array(position,2),vec_array(position,3));
...
void readGemInertial(int num_gem)
{
	...
	printf(" accelerometer sensor   [%f,%f,%f,%f]\n",vec_array(gem_inertial_state.accelerometer,0),vec_array(gem_inertial_state.accelerometer,1),vec_array(gem_inertial_state.accelerometer,2),vec_array(gem_inertial_state.accelerometer,3));

	
	printf(" accelerometer_bias sensor   [%f,%f,%f,%f]\n",vec_array(gem_inertial_state.accelerometer_bias,0),vec_array(gem_inertial_state.accelerometer_bias,1),vec_array(gem_inertial_state.accelerometer_bias,2),vec_array(gem_inertial_state.accelerometer_bias,3));
	
	printf(" gyro sensor  [%f,%f,%f,%f]\n",vec_array(gem_inertial_state.gyro,0),vec_array(gem_inertial_state.gyro,1),vec_array(gem_inertial_state.gyro,2),vec_array(gem_inertial_state.gyro,3));
	
	printf(" gyro_bias sensor  [%f,%f,%f,%f]\n",vec_array(gem_inertial_state.gyro_bias,0),vec_array(gem_inertial_state.gyro_bias,1),vec_array(gem_inertial_state.gyro_bias,2),vec_array(gem_inertial_state.gyro_bias,3));
...
void readGem()
{
		...
		{					
			...
			case 2:
				...
				printf("Frame %d center of the sphere in world coordinates %f %f %f %f \n",camread.frame,vec_array(gem_state.pos,0),vec_array(gem_state.pos,1),vec_array(gem_state.pos,2),vec_array(gem_state.pos,3)); 
				break;
			case 4:
				printf("Move pressed \n");
				gemCalibrate(0);	
			...
			case 64:
				...
				printf("X,Y,Z position (mm) %f %f %f\n",vec_array(gem_state.pos,0),vec_array(gem_state.pos,1),vec_array(gem_state.pos,2));
